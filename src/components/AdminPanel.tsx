...
import { Download, Plus, Trash, Edit, X, Users } from 'lucide-react'
...
const AdminPanel = () => {
  ...
  const exportOrders = () => {
    ...
    // Create a new array with desired columns in the new order
  ...
  const exportUserStats = () => {
    ...
    // Calculate order count and total spending per user
    const userStats = orders.reduce((acc: { [key: string]: { orderCount: number, totalSpent: number } }, order) => {
      ...
      // Find product price for this order
      const product = products.find(p => p.id === order.productId)
    ...
    // Convert to array format for Excel with Instagram username first
    ...
    // Set specific widths for user stats columns
    ws['!cols'] = [
